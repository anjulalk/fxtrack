...
import { dark, resetTheme, toggleTheme } from '../lib/theme'

const label = computed(() =>
  dark.value ? 'Switch to light appearance' : 'Switch to dark appearance',
)
</script>
...
<template>
  <button
    type="button"
    class="ui flex h-12 w-12 shrink-0 cursor-pointer items-center justify-center text-mute transition hover:text-clay-strong"
    aria-label="appearance switcher"
    :title="label"
    @click="toggleTheme"
    @contextmenu.prevent="resetTheme"
  >
    <svg v-if="dark" class="h-[1em] w-[1em]" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
    ...
    <svg v-else class="h-[1em] w-[1em]" viewBox="0 0 512 512" aria-hidden="true">
      <path
        fill="currentColor"
